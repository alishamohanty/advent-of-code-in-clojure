...
;;so recent bingo info is a 1d vec
;;so loop through recent-bingo-infos and check if the bingo-info is already present
;;if it is not present then insert it and return the new bingo info
;;it all are present then return nil


(defn insert-recent-bingo [bingo-infos recent-bingo-infos]
  (let [intersec (vec (clojure.set/intersection (set bingo-infos) (set recent-bingo-infos)))
        present (fn [recent-bingo-info] (if (some #(= (:board-idx %)
                                                      (:board-idx recent-bingo-info)) intersec)
                                          true
                                          false))
        ans (remove present recent-bingo-infos)

        _ (println "ans" ans "union" intersec)])) ;;got the union
